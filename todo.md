...
4. 🟠 **关闭窗口行为** ✅（2026-08-30）：`tray.close_to_tray` 开关 + pystray 托盘（显示窗口/退出菜单）；关窗拦截改为隐藏，托盘退出才真退出；依赖已进 requirements.txt 与 build.spec。⏳ 需桌面模式验收托盘交互。
5. 🟠 **自启动静默启动** ✅（2026-08-30）：`app.start_minimized`（依赖托盘开启），启动即隐藏窗口只在托盘待命；设置面板已做联动禁用。⏳ 需桌面模式验收。
6. 🟠 **记住窗口大小/位置** ✅（2026-08-30）：关窗写 `window.geometry`，启动还原（大小必有、位置尽力而为），读写已验证。
...
> ✅ 2026-08-30 全部批次已完成，逐条验证通过（冒烟测试 56/56）。
> 待人工验收：托盘交互与静默启动需桌面模式（`python desktop.py` 或打包 exe）确认。
